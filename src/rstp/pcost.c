...
/*kd: im 802.1D bleibt die gleiche skript*/
#include "base.h"
#include "stpm.h"
...
static long computeAutoPCost (STATE_MACH_T *self)
{
    long lret;
...
static void updPortPathCost (STATE_MACH_T *self)
{
  register PORT_T * port = self->owner.port;
  port->selected = False;
  port->reselect = True;
...
void STP_pcost_enter_state (STATE_MACH_T *self)
{
  register PORT_T*  port = self->owner.port;
  ...
  switch (self->State) {
    case BEGIN:
      break;
    case AUTO:
      port->operSpeed = STP_OUT_get_port_oper_speed (port->owner->rstp, port->port_index);
#ifdef STP_DBG
      if (port->pcost->debug) {
      ...
      port->operPCost = computeAutoPCost (self);
      break;
    case FORSE:
      ...
      break;
    case STABLE:
      updPortPathCost (self);
      break;
  }
}

Bool STP_pcost_check_conditions (STATE_MACH_T* self)
{
  register PORT_T*  port = self->owner.port;

  switch (self->State) {
    case BEGIN:
      return STP_hop_2_state (self, AUTO);
    case AUTO:
      return STP_hop_2_state (self, STABLE);
    case FORSE:
      return STP_hop_2_state (self, STABLE);
    case STABLE:
      ...
      if (ADMIN_PORT_PATH_COST_AUTO != port->adminPCost &&
          port->operPCost != port->adminPCost) {
          return STP_hop_2_state (self, FORSE);
      }
      break;
  }
  return False;
