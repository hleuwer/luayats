...
//char *basemac)
rstp_bridge::rstp_bridge(root *node, int nports, char *basemac)
{
   int i;
...
void rstp_bridge::set_speed(int port_index, unsigned long speed)
{
  this->speed[port_index - 1] = speed;
  pathcost[port_index - 1] = speed2pcost(speed);
}
...
void rstp_bridge::set_duplex(int port_index, int duplex)
{
  this->duplex[port_index-1] = duplex;
}
...
void rstp_bridge::set_port_trace(char *mach_name, int vlan_id, int port_index, int enadis)
{
  BITMAP_T ports;
  BitmapSetBit(&ports, port_index - 1);
#ifdef ULLA
  STP_IN_dbg_set_port_trace(mach_name, enadis, this, vlan_id, &ports, True);
#endif
}

void rstp_bridge::set_stpm_trace(int vlan_id, int enadis)
{
#ifdef STP_DBG
  STP_IN_dbg_set_stpm_trace(enadis, this, vlan_id, True);
#endif
}
