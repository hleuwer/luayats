...
#define ISOLATE_TRANSMIT_STPM (1)

PORT_TIMER_T TxHoldCount = 6;

static int _stp_stpm_init_machine (STATE_MACH_T* self)
{
  self->State = BEGIN;
...
static int _stp_stpm_iterate_machines (STPM_T* self,
...
#ifdef ISOLATE_TRANSMIT_STPM
      if (stater == port->transmit)
	continue;
#endif
      iret = (*iter_callb) (stater);
      if (exit_on_non_zero_ret && iret)
...
static void _stp_stpm_init_data (STPM_T* self)
{
...
static unsigned char _check_topoch (STPM_T* self)
{
  register PORT_T*  port;
...
STPM_T* STP_stpm_create (rstp_bridge *rstp, int vlan_id, char* name)
{
  STPM_T* self;
...
int STP_stpm_enable (STPM_T* self, UID_STP_MODE_T admin_state)
{
  int rc = 0;
...
void STP_stpm_delete (STPM_T* self)
{
  register STPM_T*       tmp;
...
int STP_stpm_start (STPM_T* self)
{
  register PORT_T* port;
...
void STP_stpm_stop (STPM_T* self)
{
}

#if 1
int STP_stpm_update (STPM_T* self) /* returns number of loops */
{
  register Bool     need_state_change;
  register int      number_of_loops = 0;
  register int      number_of_iterations = 0;
...
#ifdef STP_DBG
#if 0
      stp_trace("%d %d (stpm_update)", number_of_loops, number_of_iterations);
#endif
#endif
...
#ifdef STP_DBG
#if 0
  stp_trace("%d (stpm_update)", number_of_loops);
#endif
#endif
  return number_of_loops;
}

#else /* Alternative - old, not used */
...
BRIDGE_ID * STP_compute_bridge_id (STPM_T* self)
{
  register PORT_T* port;
...
STPM_T* STP_stpm_get_the_list (rstp_bridge *rstp)
{
  return rstp->bridges;
}

void STP_stpm_update_after_bridge_management (STPM_T* self)
{
  register PORT_T* port;
...
int STP_stpm_check_bridge_priority (STPM_T* self)
{
  register STPM_T* oth;
...
const char* STP_stpm_get_port_name_by_id (STPM_T* self, PORT_ID port_id)
{
  register PORT_T* port;
