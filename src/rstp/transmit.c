...
/*kd: im 802.1D in solche ZustandMashine gibt es keine neuen Funktionen. Und die Unterschied ist nicht zu Gross*/
...
static size_t build_bpdu_header (rstp_bridge *rstp, int port_index,
...
/* 17.21.21 checked: leu */
static int txTcn (STATE_MACH_T* self)
{ 
...
static void build_config_bpdu (PORT_T* port, Bool set_topo_ack_flag)
{
  ...
  STP_VECT_set_vector (&port->designPrio, &bpdu_packet.body);
  STP_set_times (&port->designTimes, &bpdu_packet.body);
}

/* 17.21.19 checked: leu */ /* old comment: (page 67) & 9.3.1 (page 23) */
static int txConfig (STATE_MACH_T* self)
{
  register size_t   pkt_len;
...
/* 17.21.20 checked: leu */ /* old comment: (page 68) & 9.3.3 (page 25) */
static int txRstp (STATE_MACH_T* self)
{
  register size_t       pkt_len;
  register PORT_T*      port = NULL;
  register int          port_index, vlan_id;
...
#ifdef STP_DBG
  if (self->debug)
    stp_trace ("port %s txRstp flags=0x%lx F:%d L:%d P:%d A:%d",
	       port->port_name,
	       (unsigned long) bpdu_packet.body.flags,
	       bpdu_packet.body.flags & FORWARD_BIT ? 1 : 0,
	       bpdu_packet.body.flags & LEARN_BIT  ? 1 : 0,
	       bpdu_packet.body.flags & PROPOSAL_BIT ? 1 : 0,
	       bpdu_packet.body.flags & AGREEMENT_BIT ? 1 : 0
	       );
#endif
  return STP_OUT_tx_bpdu (self->owner.port->owner->rstp, port_index, vlan_id,
                          (unsigned char *) &bpdu_packet,
...
void STP_transmit_enter_state (STATE_MACH_T* self)
{
  ...
  case IDLE:
    port->helloWhen = port->owner->rootTimes.HelloTime; /*802.1D*/
    break; 
...
Bool STP_transmit_check_conditions (STATE_MACH_T* self)
{
  register PORT_T*     port = self->owner.port;
  ...
  case IDLE:
    if (port->helloWhen==0) 
      return STP_hop_2_state (self, TRANSMIT_PERIODIC);
