...
// Make this module C++ compatible: this is a keyword

#define USE_ENABLE_PORT_ON_STPM (1)
...
void *stp_in_stpm_create (rstp_bridge *rstp, int vlan_id, char* name, BITMAP_T* port_bmp, int* err_code)
{
  int port_index;
  ...
  if (self) { /* it had just been created :( */
    *err_code = STP_Nothing_To_Do;
    return self;
  }

  self = STP_stpm_create (rstp, vlan_id, name);
  if (! self) { /* can't create stpm :( */
    *err_code = STP_Cannot_Create_Instance_For_Vlan;
    return NULL;
  }

  for (port_index = 1; port_index <= rstp->max_port; port_index++) {
    if (BitmapGetBit(port_bmp, (port_index - 1))) {
      if (! STP_port_create (self, port_index)) {
        /* can't add port :( */
        stp_warn ("can't create port %d", (int) port_index);
...
int _stp_in_stpm_enable (rstp_bridge *rstp, int vlan_id, char* name,
                    BITMAP_T* port_bmp,
                    UID_STP_MODE_T admin_state)
{
  ...
  if (STP_DISABLED != admin_state) {
	...
	if (STP_DISABLED != stpm->admin_state) {
	  STP_OUT_set_hardware_mode (stpm->rstp, stpm->vlan_id, STP_DISABLED);
  ...
  if (! self) { /* it had not yet been created */
	...
	stp_trace ("implicit create to vlan '%s'", name);
      ...
      if (! self) {
        stp_warn ("implicit create to vlan '%s' failed", name);
  ...
  if (self->admin_state == admin_state) { /* nothing to do :) */
    return 0;
  }
  
  rc = STP_stpm_enable (self, admin_state);
  if (! rc) {
    STP_OUT_set_hardware_mode (self->rstp, self->vlan_id, admin_state);
  }
...
STPM_T *stpapi_stpm_find (rstp_bridge *rstp, int vlan_id)
{
  register STPM_T* self;

  for (self = STP_stpm_get_the_list (rstp); self; self = self->next)
...
static PORT_T *_stpapi_port_find (STPM_T* self, int port_index)
{
...
static void _stp_in_up_port(STPM_T *stpm, int port_index, Bool enable)
{
  register PORT_T* port;
  ...
  port->macOperational = enable;
}

static void _stp_in_enable_port_on_stpm (STPM_T* stpm, int port_index, Bool enable)
{
  register PORT_T* port;

  port = _stpapi_port_find (stpm, port_index);
  if (! port) return; 
  if (portEnabled(port) == enable) {/* nothing to do :) */
    return;
  }
...
void STP_IN_init (rstp_bridge *rstp, int max_port_index)
{
  rstp->max_port = max_port_index;
  RSTP_INIT_CRITICAL_PATH_PROTECTIO;
}

int STP_IN_stpm_get_cfg (IN rstp_bridge *rstp, int vlan_id, OUT UID_STP_CFG_T* uid_cfg)
{
  register STPM_T* self;
  ...
  if (self->admin_state != STP_DISABLED) {
    uid_cfg->field_mask |= BR_CFG_STATE;
  }
  uid_cfg->stp_enabled = self->admin_state;

  if (self->ForceVersion != 2) {
    uid_cfg->field_mask |= BR_CFG_FORCE_VER;
  }
  uid_cfg->force_version = self->ForceVersion;

  if (self->BrId.prio != DEF_BR_PRIO) {
    uid_cfg->field_mask |= BR_CFG_PRIO;
  }
  uid_cfg->bridge_priority = self->BrId.prio;

  if (self->BrTimes.MaxAge != DEF_BR_MAXAGE) {
    uid_cfg->field_mask |= BR_CFG_AGE;
  }
  uid_cfg->max_age = self->BrTimes.MaxAge;

  if (self->BrTimes.HelloTime != DEF_BR_HELLOT) {
    uid_cfg->field_mask |= BR_CFG_HELLO;
  }
  uid_cfg->hello_time = self->BrTimes.HelloTime;

  if (self->BrTimes.ForwardDelay != DEF_BR_FWDELAY) {
    uid_cfg->field_mask |= BR_CFG_DELAY;
  }
  uid_cfg->forward_delay = self->BrTimes.ForwardDelay;
...
int STP_IN_port_get_state (IN rstp_bridge *rstp, int vlan_id, INOUT UID_STP_PORT_STATE_T* entry)
{
  register STPM_T* self;
  register PORT_T* port;
  ...
  port = _stpapi_port_find (self, entry->port_no);
  if (! port) {/* port is absent in the stpm :( */
    RSTP_CRITICAL_PATH_END;
    return STP_Port_Is_Absent_In_The_Vlan;
  }
  strncpy(entry->vlan_name, self->name, NAME_LEN);
...
int STP_IN_stpm_get_state (IN rstp_bridge *rstp, int vlan_id, OUT UID_STP_STATE_T* entry)
{
  register STPM_T* self;
  ...
  strncpy (entry->vlan_name, self->name, NAME_LEN);
  entry->vlan_id = self->vlan_id;
  _conv_br_id_2_uid (&self->rootPrio.root_bridge, &entry->designated_root);
  entry->root_path_cost = self->rootPrio.root_path_cost;
  entry->root_port = self->rootPortId;
  entry->max_age =       self->rootTimes.MaxAge;
  entry->forward_delay = self->rootTimes.ForwardDelay;
  entry->hello_time =    self->rootTimes.HelloTime;

  _conv_br_id_2_uid (&self->BrId, &entry->bridge_id);

  entry->stp_enabled = self->admin_state;

  entry->timeSince_Topo_Change = self->timeSince_Topo_Change;
  entry->Topo_Change_Count = self->Topo_Change_Count;
  entry->Topo_Change = self->Topo_Change;
  
  RSTP_CRITICAL_PATH_END;
  return 0;
}

int STP_IN_stpm_get_name_by_vlan_id (rstp_bridge *rstp, int vlan_id, char* name, size_t buffsize)
{
  register STPM_T* stpm;
  int iret = -1;
...
int STP_IN_up_port(rstp_bridge *rstp, int port_index, Bool enable)
{
  register STPM_T* stpm;

  RSTP_CRITICAL_PATH_START;
  rstp->tev = enable ? RSTP_PORT_EN_T : RSTP_PORT_DIS_T; INCR100(rstp->nev);

  if (! enable) {
#ifdef STP_DBG
    if (rstp->debug)
      stp_trace("%s (p%02d, all, %s, '%s')",
		"clearFDB", (int) port_index, "self port", "down port");
#endif
#if 0
    STP_OUT_flush_lt (rstp, port_index, 0, LT_FLASH_ONLY_THE_PORT, "down port");
#endif
  }
...
#if (USE_ENABLE_PORT_ON_STPM == 1)
    _stp_in_enable_port_on_stpm (stpm, port_index, enable);
#else
    _stp_in_up_port(stpm, port_index, enable);
#endif
...
/* call it, when link Up/Down leu: NO us STP_IN_up_port instead */
int STP_IN_enable_port (rstp_bridge *rstp, int port_index, Bool enable)
{
  register STPM_T* stpm;
  ...
  if (! enable) {
    ...
    if (rstp->debug)
      stp_trace("%s (p%02d, all, %s, '%s')",
		"clearFDB", (int) port_index, "self port", "disable port");
#endif
#if 0
    STP_OUT_flush_lt (rstp, port_index, 0, LT_FLASH_ONLY_THE_PORT, "disable port");
#endif
  }
...
/* call it, when port speed has been changed, speed in Mb/s  */
int STP_IN_changed_port_speed (rstp_bridge *rstp, int port_index, long speed)
{
  register STPM_T* stpm;
  register PORT_T* port;

  RSTP_CRITICAL_PATH_START;
  rstp->tev = RSTP_PORT_SPEED_T; INCR100(rstp->nev);
...
int STP_IN_changed_port_duplex (rstp_bridge *rstp, int port_index)
{
  register STPM_T* stpm;
  register PORT_T* port;

  RSTP_CRITICAL_PATH_START;
  rstp->tev = RSTP_PORT_DPLEX_T; INCR100(rstp->nev);
...
int STP_IN_check_bpdu_header (BPDU_T* bpdu, size_t len)
{
  ...
  if ((unsigned int) (len8023 + 14) > len) { /* len8023 format gt len :( */
  ...
  if (bpdu->eth.dsap != BPDU_L_SAP                 ||
      ...
      bpdu->eth.llc != LLC_UI) {
    /* self is not a proper 802.3 pkt! :( */
...
int STP_IN_rx_bpdu (rstp_bridge *rstp, int vlan_id, int port_index, BPDU_T* bpdu, size_t len)
{
  register PORT_T* port;
  register STPM_T* self;
  int              iret;
  ...
  RSTP_CRITICAL_PATH_START;
  rstp->tev = RSTP_PORT_RX_T; INCR100(rstp->nev);

  self = stpapi_stpm_find (rstp, vlan_id);
  if (! self) { /*  the stpm had not yet been created :( */
    RSTP_CRITICAL_PATH_END;
    return STP_Vlan_Had_Not_Yet_Been_Created;
  }

  if (STP_DISABLED == self->admin_state) {/* the stpm had not yet been enabled :( */
    RSTP_CRITICAL_PATH_END;
    return STP_Had_Not_Yet_Been_Enabled_On_The_Vlan;
  }

  port = _stpapi_port_find (self, port_index);
  if (! port) {/* port is absent in the stpm :( */
    stp_warn ("RX bpdu vlan_id=%d port=%d port is absent in the stpm :(", 
  ...
  if (port->operEdge && ! port->lnkWhile && portEnabled(port)) {
#ifdef STP_DBG
...
#if 0 /* leu: do we need this ? */
  if (! portEnabled(port)) {/* port link change indication will come later :( */
    _stp_in_enable_port_on_stpm (self, port->port_index, True);
  }
#endif  
#ifdef STP_DBG
  if (port->brdec->debug && port->operEdge) {
...
int STP_IN_one_second (rstp_bridge *rstp)
{
  register STPM_T* stpm;
  register int     dbg_cnt = 0;

  RSTP_CRITICAL_PATH_START;
  rstp->tev = RSTP_PORT_TIME_T; INCR100(rstp->nev);
...
int STP_IN_stpm_set_cfg (IN rstp_bridge *rstp, int vlan_id,
                     ...
                     IN UID_STP_CFG_T* uid_cfg)
{
  ...
  if (0 != STP_IN_stpm_get_cfg (rstp, vlan_id, &old)) {
    STP_OUT_get_init_stpm_cfg (rstp, vlan_id, &old);
  }
  
  RSTP_CRITICAL_PATH_START;
  rstp->tev = RSTP_PORT_MNGR_T; INCR100(rstp->nev);
  ...
  if (BR_CFG_HOLD_TIME & uid_cfg->field_mask) {
    old.hold_time = uid_cfg->hold_time;
  }
  ...
  if (0 != rc) {
    stp_warn ("_check_stpm_config failed %d", (int) rc);
    RSTP_CRITICAL_PATH_END;
    return rc;
  }
  ...
  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
    ...
    rc = _stp_in_stpm_enable (rstp, vlan_id, uid_cfg->vlan_name, port_bmp, STP_DISABLED);
    if (0 != rc) {
  ...
  enabled_here = False;
  if (! self) { /* it had not yet been created */
    self = (STPM_T*) stp_in_stpm_create (rstp, vlan_id, uid_cfg->vlan_name, port_bmp, &err_code);/*STP_IN_stpm_set_cfg*/
    if (! self) {
      RSTP_CRITICAL_PATH_END;
      return err_code;
    }
  ...
  prev_prio = self->BrId.prio;
  self->BrId.prio = old.bridge_priority;
  if (STP_ENABLED == self->admin_state) {
    if (0 != STP_stpm_check_bridge_priority (self)) {
      self->BrId.prio = prev_prio;
      stp_warn ("%s", "STP_stpm_check_bridge_priority failed");
      RSTP_CRITICAL_PATH_END;
      return STP_Invalid_Bridge_Priority;
    }
  }

  self->BrTimes.MaxAge = old.max_age;
  self->BrTimes.HelloTime = old.hello_time;
  self->BrTimes.ForwardDelay = old.forward_delay;
  self->ForceVersion = (PROTOCOL_VERSION_T) old.force_version;
  //  TxHoldCount = old.hold_time;

  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
      STP_DISABLED != uid_cfg->stp_enabled &&
      STP_DISABLED == self->admin_state) {
    rc = _stp_in_stpm_enable (rstp, vlan_id, uid_cfg->vlan_name, port_bmp, uid_cfg->stp_enabled);
    if (0 != rc) {
      stp_warn ("%s", "cannot enable");
      if (created_here) {
        STP_stpm_delete (self);
      }
      RSTP_CRITICAL_PATH_END;
  ...
  if (! enabled_here && STP_DISABLED != self->admin_state) {
    STP_stpm_update_after_bridge_management (self);
  }
  RSTP_CRITICAL_PATH_END;
  return 0;
}

int STP_IN_set_port_cfg (IN rstp_bridge *rstp, int vlan_id, IN UID_STP_PORT_CFG_T* uid_cfg)
{
  register STPM_T* self;
  register PORT_T* port;
  ...
  for (port_no = 1; port_no <= rstp->max_port; port_no++) {
    if (! BitmapGetBit(&uid_cfg->port_bmp, port_no - 1)) continue;
  
    port = _stpapi_port_find (self, port_no);
    if (! port) {/* port is absent in the stpm :( */
      continue;
    }

    if (PT_CFG_MCHECK & uid_cfg->field_mask) {
      if (self->ForceVersion >= NORMAL_RSTP)
        port->mcheck = True;
    }
...
int STP_IN_dbg_set_port_trace (char* mach_name, int enadis,
                           rstp_bridge *rstp, int vlan_id, BITMAP_T* ports,
                           int is_print_err)
{
  register STPM_T* self;
  register PORT_T* port;
...
int STP_IN_dbg_set_stpm_trace(int enadis, rstp_bridge *rstp, int vlan_id, int is_print_err)
{
  STPM_T *self = stpapi_stpm_find (rstp, vlan_id);
  self->debug = enadis;
  return 0;
}
#endif
