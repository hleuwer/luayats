...
**************************************************************************/
/** basic types. **/
typedef int     Int;		/* to align with convention used here for use of case. Types and defined constants have 
				   their initial letters capitalized. */
typedef Int     Boolean;	/* : (True, False) */
typedef Int     State;		/* : (Disabled, Listening, Learning, Forwarding, Blocking) */
...
Int             port_no;
{
  ...
  /* (8.6.1.3.2(c)) */
    ...
    config_bpdu[port_no].topology_change_acknowledgment = port_info[port_no].topology_change_acknowledge;
  /* (8.6.1.3.2(h)) */
...
{				/* (8.6.8) */
  ...
  for (port_no = One; port_no <= No_of_ports; port_no++) {	/* (8.6.8.3.1) */
	...
	&& ((root_port == No_port)
	    ...
	    || ((port_info[port_no].designated_root == port_info[root_port].designated_root)
		&& (((port_info[port_no].designated_cost + port_info[port_no].path_cost)
		     < (port_info[root_port].designated_cost + port_info[root_port].path_cost)	/* (8.6.8.3.1(b)) */
		    )
		    || (((port_info[port_no].designated_cost + port_info[port_no].path_cost)
			 == (port_info[root_port].designated_cost + port_info[root_port].path_cost)
			)
			&& ((port_info[port_no].designated_bridge < port_info[root_port].designated_bridge)	/* (8.6.8.3.1(c)) 
														 */
			    ||((port_info[port_no].designated_bridge == port_info[root_port].designated_bridge)
			       && ((port_info[port_no].designated_port < port_info[root_port].designated_port)	/* (8.6.8.3.1(d)) 
														 */
				   ||((port_info[port_no].designated_port == port_info[root_port].designated_port)
  ...
  for (port_no = One; port_no <= No_of_ports; port_no++) {	/* (8.6.9.3) */
	...
	||((bridge_info.root_path_cost == port_info[port_no].designated_cost)
	   && ((bridge_info.bridge_id < port_info[port_no].designated_bridge)	/* (8.6.9.3 d) */
	       ||((bridge_info.bridge_id == port_info[port_no].designated_bridge)
...
{
  if (topology_change_timer.active && (++topology_change_timer.value >= bridge_info.topology_change_time)
      ) {
    topology_change_timer.active = False;
    return (True);
  }
...
Int port_no;
{
  if (hold_timer[port_no].active && (++hold_timer[port_no].value >= bridge_info.hold_time)) {
    hold_timer[port_no].active = False;
    return (True);
  }
  return (False);
