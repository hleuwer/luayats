...
//	data item has arrived.
//	Send a clone to each output.
rec_typ dfork::REC(data * pd, int) 
{
  int n;

  // first send the clones, since pd might be deleted by
  // the receiver
  
  for (n = 1; n < noutp; ++n)
    sucs[n]->rec(pd->clone(), shands[n]);
  
  // original data object sent to first output
  sucs[0]->rec(pd, shands[0]);
