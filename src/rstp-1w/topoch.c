...
#else
static Bool
flush (STATE_MACH_T *self, char* reason) /* 17.19.9 */
{
...
static void
setTcPropBridge (STATE_MACH_T* self, char* reason) /* 17.19.14 */
{
  register PORT_T* port = self->owner.port;
  register PORT_T* tmp;
...
static unsigned int
newTcWhile (STATE_MACH_T* self) /* 17.19.7 */
{
  register PORT_T* port = self->owner.port;
...
void
STP_topoch_enter_state (STATE_MACH_T* self)
{
  register PORT_T*      port = self->owner.port;
  ...
  if (self->debug) 
    ...
    case NOTIFIED_TC:
      ...
      setTcPropBridge (self, "NOTIFIED_TC");
      break;
    case PROPAGATING:
      port->tcWhile = newTcWhile (self);
#ifdef STP_DBG
...
Bool
STP_topoch_check_conditions (STATE_MACH_T* self)
{
  register PORT_T*      port = self->owner.port;

  if (BEGIN == self->State) {
    return STP_hop_2_state (self, INIT);
  }
  ...
  switch (self->State) {
    ...
    case INACTIVE:
      if (port->role == RootPort || port->role == DesignatedPort)
        return STP_hop_2_state (self, TCACTIVE);
