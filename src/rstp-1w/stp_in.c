...
void *
stp_in_stpm_create (rstp_bridge *rstp, int vlan_id, char* name, BITMAP_T* port_bmp, int* err_code)
{
  int port_index;
...
int
_stp_in_stpm_enable (rstp_bridge *rstp, int vlan_id, char* name,
                    BITMAP_T* port_bmp,
...
STPM_T *
stpapi_stpm_find (rstp_bridge *rstp, int vlan_id)
{
  register STPM_T* self;
...
static PORT_T *
_stpapi_port_find (STPM_T* self, int port_index)
{
...
void 
STP_IN_init (rstp_bridge *rstp, int max_port_index)
{
...
int
STP_IN_stpm_get_cfg (IN rstp_bridge *rstp, int vlan_id, OUT UID_STP_CFG_T* uid_cfg)
{
  register STPM_T* self;
...
int
STP_IN_port_get_state (IN rstp_bridge *rstp, int vlan_id, INOUT UID_STP_PORT_STATE_T* entry)
{
  register STPM_T* self;
  register PORT_T* port;
...
int
STP_IN_stpm_get_state (IN rstp_bridge *rstp, int vlan_id, OUT UID_STP_STATE_T* entry)
{
  register STPM_T* self;
...
int
STP_IN_stpm_get_name_by_vlan_id (rstp_bridge *rstp, int vlan_id, char* name, size_t buffsize)
{
  register STPM_T* stpm;
  int iret = -1;
...
STP_IN_enable_port (rstp_bridge *rstp, int port_index, Bool enable)
{
  register STPM_T* stpm;
...
STP_IN_changed_port_speed (rstp_bridge *rstp, int port_index, long speed)
{
  register STPM_T* stpm;
  register PORT_T* port;
...
STP_IN_changed_port_duplex (rstp_bridge *rstp, int port_index)
{
  register STPM_T* stpm;
  register PORT_T* port;
...
int
STP_IN_rx_bpdu (rstp_bridge *rstp, int vlan_id, int port_index, BPDU_T* bpdu, size_t len)
{
  register PORT_T* port;
  register STPM_T* self;
  int              iret;
...
int
STP_IN_one_second (rstp_bridge *rstp)
{
  register STPM_T* stpm;
...
int
STP_IN_stpm_set_cfg (IN rstp_bridge *rstp, int vlan_id,
...
int
STP_IN_set_port_cfg (IN rstp_bridge *rstp, int vlan_id, IN UID_STP_PORT_CFG_T* uid_cfg)
{
  register STPM_T* self;
  register PORT_T* port;
