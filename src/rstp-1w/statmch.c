...
void
STP_state_mach_delete (STATE_MACH_T *self)
{
  free (self->name);
...
Bool
STP_check_condition (STATE_MACH_T* self)
{
  Bool bret;
...
Bool
STP_change_state (STATE_MACH_T* self)
{
...
Bool
STP_hop_2_state (STATE_MACH_T* self, unsigned int new_state)
{
#ifdef STP_DBG
  switch (self->debug) {
    case 0: break;
    case 1:
      if (new_state == self->State || new_state == self->ignoreHop2State) break;
