...
#ifdef __BITMAP_H
int
STP_IN_stpm_create (rstp_bridge *rstp, int vlan_id, char* name, BITMAP_T* port_bmp);
#endif

int
STP_IN_stpm_delete (rstp_bridge *rstp, int vlan_id);

int
STP_IN_stop_all (rstp_bridge *rstp);

int
STP_IN_delete_all (rstp_bridge *rstp);

/* Section 2. "Get" management */

Bool
STP_IN_get_is_stpm_enabled (rstp_bridge *rstp, int vlan_id);

int
STP_IN_stpm_get_vlan_id_by_name (rstp_bridge *rstp, char* name, int* vlan_id);
...
#ifdef _UID_STP_H__
int
STP_IN_stpm_get_cfg (rstp_bridge *rstp, int vlan_id, UID_STP_CFG_T* uid_cfg);

int
STP_IN_stpm_get_state (rstp_bridge *rstp, int vlan_id, UID_STP_STATE_T* entry);

int
STP_IN_port_get_cfg (rstp_bridge *rstp, int vlan_id, int port_index, UID_STP_PORT_CFG_T* uid_cfg);

int
STP_IN_port_get_state (rstp_bridge *rstp, int vlan_id, UID_STP_PORT_STATE_T* entry);
#endif

/* Section 3. "Set" management */

int
STP_IN_stpm_set_cfg (rstp_bridge *rstp, int vlan_id,
                     BITMAP_T* port_bmp,
                     UID_STP_CFG_T* uid_cfg);

int
STP_IN_set_port_cfg (rstp_bridge *rstp, int vlan_id,
