...
--require "base"
module ('gui.lexer',package.seeall)
...
local function tdump(tok,options,b,e)
	return yield(tok,tok,b,e)
end

local function ndump(tok,options,b,e)
   if options and options.number then
      tok = tonumber(tok)
   end
   return yield("number",tok,b,e)
end

local function sdump(tok,options, b, e)
   if options and options.string then
      tok = tok:sub(2,-2)
   end
   return yield("string",tok,b,e)
end

local function chdump(tok,options,b,e)
   if options and options.string then
      tok = tok:sub(2,-2)
   end
   return yield("char",tok,b,e)
end

local function cdump(tok,options, b,e)
   return yield("comment",tok,b,e)
end

local function wsdump (tok,options, b,e)
    return yield("space",tok,b,e)
end

local function plain_vdump(tok,options, b,e)
    return yield("iden",tok,b,e)
end

local function lua_vdump(tok,options, b,e)
    if lua_keyword[tok] then
        return yield("keyword",tok,b,e)
    else
        return yield("iden",tok,b,e)
    end
end

local function cpp_vdump(tok,options, b,e)
...
--      print('sz',sz)
	 ...
	 for _,m in ipairs(matches) do
	    ...
	    if i1 then
		  ...
		  --print(s,pat,idx,tok)
		  finished = idx > sz
		  res1,res2 = fun(tok,options,i1,i2)
		  --print("#res", res1,res2)
	       end
	       if res1 then
		  -- insert a token list
...
--[[
	 ...
	 ["dofile"] = true,  ["error"] = true,
	 ["print"] = true,  ["tostring"] = true,
	 ["tonumber"] = true,  ["getfenv"] = true,
	 ["assert"] = true,  ["collectgarbage"] = true,
	 ["setfenv"] = true,  ["load"] = true,
	 ["loadfile"] = true,  ["loadstring"] = true,
	 ["rawequal"] = true,  ["rawget"] = true,
	 ["rawset"] = true,  ["setmetatable"] = true,
	 ["getmetatable"] = true,  ["pcall"] = true,
	 ["xpcall"] = true,  ["type"] = true
   ]]
      }
   end
