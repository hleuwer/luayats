...
print("Connections of mux")
for i, v in dmx:successors() do
   print(string.format("dmx %d: %d %s", i, v.shand, v.suc.name))
end
for i, v in mx:successors() do
   print(string.format("mux %d: %d %s", i, v.shand, v.suc.name))
end
for i = 1, nsrc do
   for j, v in src[i]:successors() do
      print(string.format("src[%d] %d: %d %s %s", i, j, v.shand, v.suc.name, v.suc.inputs[v.shand+1]))
   end
end
