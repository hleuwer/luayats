...
yats.cdebug = false
-- Create Multiplexer
mx = yats.muxWFQ{"mux", ninp=nsrc, out={"demux", "demux"}}
for i = 1,nsrc do
  local delta = 5
  if i < 4 then delta = 1 end
  mx:addVCQ{vc=i, delta=delta, qsize=10}
end

print("WFQ Mux parameters:")
for i = 1, nsrc do 
  local par = mx:getVCQ(i)
  print(string.format("  vc %d: delta=%d qsize=%d name=%s", i, par.delta, par.qsize, par.name or "unkown"))
end
